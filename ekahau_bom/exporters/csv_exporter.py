...
class CSVExporter(BaseExporter):
    ...
    def _export_access_points(
        ...
        For detailed per-AP information, see _access_points_detailed.csv.
        ...
        with open(output_file, "w", newline="", encoding="utf-8") as f:
            ...
            # Write header - simple BOM with vendor, model, quantity only
            writer.writerow(["Vendor", "Model", "Quantity"])

            # Write data rows - sorted by vendor, then model
            for (vendor, model), count in sorted(ap_counts.items()):
                writer.writerow([vendor, model, count])
