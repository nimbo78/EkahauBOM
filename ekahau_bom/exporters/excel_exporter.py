...
class ExcelExporter(BaseExporter):
    ...
    def _create_access_points_sheet(
        ...
        """Create access points BOM sheet.

        Creates a simple BOM (Bill of Materials) for purchasing, grouped by vendor and model only.
        For detailed per-AP information, see "AP Installation Details" sheet.
        ...
        # Headers - simple BOM with vendor, model, quantity only
        headers = ["Vendor", "Model", "Quantity"]
        ws.append(headers)
        self._apply_header_style(ws)

        # Simple BOM for purchasing - group by vendor and model only
        ap_tuples = [(ap.vendor, ap.model) for ap in access_points]
        ...
        # Write data - sorted by vendor, then model
        for (vendor, model), count in sorted(ap_counts.items()):
            ws.append([vendor, model, count])
    ...
    def _create_detailed_access_points_sheet(
        ...
        for ap in access_points:
            ...
            mounting_height = self._get_mounting_height(ap, radios)
