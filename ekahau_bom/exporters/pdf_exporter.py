...
class PDFExporter(BaseExporter):
    ...
    - Floor plan visualizations (optional)
    - Print-optimized layout

    Requires: WeasyPrint library
    """

    def __init__(self, output_dir: Path, visualization_dir: Optional[Path] = None):
        """Initialize PDF exporter.

        Args:
            output_dir: Directory where PDF files will be saved
            visualization_dir: Optional directory containing floor plan PNGs
        """
    ...
    def _generate_pdf_html(self, project_data: ProjectData) -> str:
        ...
        floor_plans_html = self._generate_floor_plans_section(project_data)
...
<body>
    ...
    * {
    ...
    def _generate_summary(
        ...
        if metadata:
            metadata_items = []
            # Always add project_name (parameter already has correct priority: title → name → filename)
            metadata_items.append(
                f"<p><strong>Project Name:</strong> {html_module.escape(project_name)}</p>"
            )
    ...
    def _generate_detailed_aps_table(
        ...
        for ap in access_points:
            ...
            height = f"{mounting_height_value:.2f}" if mounting_height_value is not None else "N/A"
            azimuth = (
    ...
    def _generate_floor_plans_section(self, project_data: ProjectData) -> str:
        """Generate floor plan visualizations section.

        Embeds floor plan PNG images sorted by floor number (bottom to top).

        Args:
            project_data: Project data containing floor information

        Returns:
            HTML string for floor plan visualizations section
        """
        if not self.visualization_dir or not self.visualization_dir.exists():
            return ""

        # Get all floor plan PNG files
        floor_plan_files = list(self.visualization_dir.glob("*.png"))
        if not floor_plan_files:
            return ""

        # Create mapping of floor ID to floor data for sorting
        floor_data_map = {}
        for floor_id, floor in project_data.floors.items():
            # Find matching PNG file (format: floor_name_visualization.png)
            matching_file = None
            for fp_file in floor_plan_files:
                # Remove "_visualization" suffix from stem for matching
                file_stem = fp_file.stem.replace("_visualization", "")
                # Match by floor name (sanitized) or floor ID
                safe_floor_name = self._sanitize_filename(floor.name)
                if file_stem == safe_floor_name or file_stem == floor_id:
                    matching_file = fp_file
                    break
        ...
        # Sort floors by floor_number (ascending: basement to top floor)
        sorted_floors = sorted(floor_data_map.values(), key=lambda x: x["floor"].floor_number)

        # Generate HTML section
        html = '<section class="floor-plans"><h3>Floor Plan Visualizations</h3>'

        for floor_info in sorted_floors:
            floor = floor_info["floor"]
            floor_file = floor_info["file"]

            # Read and encode image as base64
            try:
                with open(floor_file, "rb") as img_file:
                    img_data = img_file.read()
                    img_base64 = base64.b64encode(img_data).decode("utf-8")

                # Add floor heading and image
                html += f'<div class="floor-plan-item">'
                html += f"<h4>{html_module.escape(floor.name)} (Floor {floor.floor_number})</h4>"
                html += f'<img src="data:image/png;base64,{img_base64}" alt="{html_module.escape(floor.name)}" class="floor-plan-image" />'
                html += f"</div>"
            except Exception as e:
                logger.warning(f"Failed to embed floor plan image {floor_file}: {e}")
                continue
