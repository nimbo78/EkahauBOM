...
class Config:
        ...
        if 'export' in self._data and 'formats' in self._data['export']:
            ...
            for fmt in formats:
                if fmt not in valid_formats:
                    raise ConfigError(f"Invalid export format: {fmt}. Must be one of: {valid_formats}")
        ...
        if 'grouping' in self._data and 'group_by' in self._data['grouping']:
            ...
            if group_by is not None:
                ...
                if group_by not in valid_dimensions:
                    raise ConfigError(f"Invalid grouping dimension: {group_by}. Must be one of: {valid_dimensions}")
        ...
        if 'pdf' in self._data:
            if 'paper_size' in self._data['pdf']:
                ...
                if paper_size not in valid_sizes:
                    raise ConfigError(f"Invalid PDF paper size: {paper_size}. Must be one of: {valid_sizes}")

            if 'orientation' in self._data['pdf']:
                orientation = self._data['pdf']['orientation']
                valid_orientations = {'portrait', 'landscape'}
                if orientation not in valid_orientations:
                    raise ConfigError(f"Invalid PDF orientation: {orientation}. Must be one of: {valid_orientations}")
    ...
    def merge_with_args(self, args: Any) -> dict[str, Any]:
        ...
        # Pricing
        merged['enable_pricing'] = getattr(args, 'enable_pricing', False) or self.get('pricing.enabled', False)
        ...
        merged['no_volume_discounts'] = getattr(args, 'no_volume_discounts', False) or not self.get('pricing.volume_discounts', True)

        # Filters
        merged['filter_floors'] = self._merge_list_arg(args, 'filter_floor', self.get('filters.include_floors', []))
        merged['filter_colors'] = self._merge_list_arg(args, 'filter_color', self.get('filters.include_colors', []))
        merged['filter_vendors'] = self._merge_list_arg(args, 'filter_vendor', self.get('filters.include_vendors', []))
        merged['filter_models'] = self._merge_list_arg(args, 'filter_model', self.get('filters.include_models', []))
        merged['filter_tags'] = getattr(args, 'filter_tag', None) or self.get('filters.tags', None)

        merged['exclude_floors'] = self._merge_list_arg(args, 'exclude_floor', self.get('filters.exclude_floors', []))
        merged['exclude_colors'] = self._merge_list_arg(args, 'exclude_color', self.get('filters.exclude_colors', []))
        merged['exclude_vendors'] = self._merge_list_arg(args, 'exclude_vendor', self.get('filters.exclude_vendors', []))
