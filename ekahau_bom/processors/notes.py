...
class NotesProcessor:
    ...
    def process_cable_notes(
        ...
        for cable_note_data in cable_notes_list:
            try:
                ...
                logger.warning(f"Failed to process cable note {cable_note_data.get('id', 'unknown')}: {e}")
                continue
    ...
    def process_picture_notes(
        self,
        picture_notes_data: dict[str, Any],
        floors: dict[str, Floor]
    ) -> list[PictureNote]:
        ...
        for picture_note_data in picture_notes_list:
            try:
                ...
                logger.warning(f"Failed to process picture note {picture_note_data.get('id', 'unknown')}: {e}")
                continue

        logger.info(f"Processed {len(processed_picture_notes)}/{len(picture_notes_list)} picture notes")
        return processed_picture_notes

    def _process_single_picture_note(
        self,
        picture_note_data: dict[str, Any],
        floors: dict[str, Floor]
    ) -> PictureNote:
